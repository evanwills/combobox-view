...
/**
 * Try (and try again) to fetch data from the server and save it to
 * the VueX store.
 *
 * @param {Array|Object|null} data     Vue component ref() constant
 *                                     Used to check whether fetching
 *                                     is required
 * @param {function}          updater  Function to update ref values
 *                                     in vue component
 * @param {function}          getter   service method used to get
 *                                     data from server
 * @param {string}            catchMsg string to append to Promise
 *                                     catch error message
 * @param {number}            delay    Number of milliseconds to
 *                                     delay before next try
 * @param {number}            tryCount Maximum number of retrys
 *                                     before giving up on fetch
 */
    ...
    if ((data.value === null
      ...
      setTimeout(() => {
        getter().then((response) => {
          ...
          if (isObj(response)) {
            // This is good. We got something meaningful back from
            // the server

            updater('main', response);
            updater('loading', false);
          } else if (tries > 0) {
            // Bummer we didn't get what we wanted, lets try again.
            retry();
          } else {
            // I give up.
            // We've tried a few times but haven't got anything
            // sensible!!!
            // eslint-disable-next-line no-console
            console.error(
              `retryFetch.retry() response was undefined ${tryCount}`
              + ` times when trying to fetch data for ${catchMsg}`,
            );
            updater('loading', false);
            updater('failed', true);
          }
        }).catch((error) => {
          console.error(`Error getting data for ${catchMsg}`, error);
          updater('loading', false);
          updater('failed', true);
        });
      }, delay);
...
/**
 * Try (and try again) to fetch data from the server and do something
 * with it
 *
 * @param {function}          getter   service method used to get
 *                                     data from server
 * @param {function}          success  Function to do something with
 *                                     the successfully fetched data
 * @param {function}          fail     Function to do something when
 *                                     the request fails, either by
 *                                     request error or too many
 *                                     unsuccesful attempts
 * @param {string}            catchMsg Name of the service function
 *                                     called to fetch data from the
 *                                     server
 * @param {function}          tryAgain Function that indicates
 *                                     whether or not to keep trying
 * @param {number}            delay    Number of milliseconds to
 *                                     delay before next try
 * @param {number}            tryCount Maximum number of retrys
 *                                     before giving up on fetch
 */
export const localRetryFetch = (
  getter,
  success,
  fail,
  catchMsg = '',
  ...
  const retry = () => {
    if (tryAgain() === true && tries > 0 && waiting === false) {
      waiting = true;
      tries -= 1;
      ...
      setTimeout(() => {
        getter().then((response) => {
          ...
          if (isObj(response)) {
            ...
            success(respAdapter(response));
          } else if (tries > 0) {
            // Bummer we didn't get what we wanted, lets try again.
            retry();
          } else {
            // We give up.
            // We've tried a few times but haven't gotten anything
            // sensible!

            console.error( // eslint-disable-next-line no-console
              'localRetryFetch.retry() response was undefined '
              + `${tryCount} times when trying to fetch data for `
              + `${catchMsg}`,
            );
            fail();
          }
  ...
  // Lets try out luck and see if we get data on the first attempt
  getter().then((response) => {
    if (isObj(response)) {
      // Yay! We got data on the first try
      success(response);
    } else {
      // Oh well... no luck on the first attempt.
